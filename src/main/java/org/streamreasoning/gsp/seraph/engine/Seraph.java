...
import org.streamreasoning.gsp.seraph.engine.windowing.SeraphTimeWindowOperatorFactory;
...
public class Seraph implements QueryRegistrationFeature<SeraphQL>, StreamRegistrationFeature<PGStream, PGStream> {
    ...
    protected Map<String, SeraphQL> registeredQueries;
    ...
    @Override
    public ContinuousQueryExecution<PGraph, PGraph, Map<String, Object>, Map<String, Object>> register(SeraphQL q) {
        ...
        q.getWindowMap().values().forEach(s -> {
            PGStream register = this.register((PGStream) s);
            in.add(register);
        });
...
//        ContinuousQueryExecution<PGraph, PGraph, Map<String, Object>, SeraphBinding> cqe = new Neo4jContinuousQueryExecutionImpl<PGraph,PGraph,Map<String, Object>,SeraphBinding>(sds, q, out, in, r2r,r2s);
        ...
        windowMap.forEach((WindowNode wo, PGStream s) -> {
            ...
            if (registeredStreams.contains(s)) {
                ...
                StreamToRelationOp<PGraph, PGraph> wop = ((SeraphStreamToRelationOp) build).link(cqe, db);


                //check if s (stream out of the windowmap) equals to an input stream, if yes, apply a tvg and add it to a sds
                List<DataStream<PGraph>> stream1 = in.stream().toList();
                stream1.stream().filter(stream -> s.getName().equals(stream.getName())).forEach(stream -> {
    ...
    public void unregisterQuery(String qid){
        ContinuousQueryExecution remove = queryExecutions.remove(qid);
    }
    ...
    public Optional<PGStream> get(String s) {
        return registeredStreams.stream().filter(strm -> s.equals(strm.getName())).findFirst();
    }
