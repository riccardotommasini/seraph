...
public class Neo4jContinuousQueryExecutionImpl<I, W, R, O> extends Neo4jContinuousQueryExecutionObserver<I, W, R, O> {
    ...
    public Neo4jContinuousQueryExecutionImpl(SDS sds, ContinuousQuery<I, W, R, O> query, DataStream<O> outstream, List<DataStream<PGraph>> instreams, RelationToRelationOperator<W, R> r2r, RelationToStreamOperator<R, O> r2s, StreamToRelationOp<I, W>... s2rs) {
        super(sds, query);
        this.s2rs = new ArrayList<>();
        this.s2rs.addAll(Arrays.asList(s2rs));
        this.query = query;
        this.sds = sds;
        this.r2r = r2r;
        this.r2s = r2s;
    ...
    public void update(Observable o, Object arg) {
        ...
        Long now = (Long) window.getC();
        Stream<R> eval1 = eval(now);
        Stream<Map<String, Object>> myEval = (Stream<Map<String, Object>>) eval1;
        Stream<R> eval2 = (Stream<R>)
                myEval.map(map -> {
                    map.put("win_start", window.getO());
                    map.put("win_end", window.getC());
                    return map;
                });
        Stream<O> eval = r2s.eval(eval2, now);
        eval.forEach(o1 -> {
            outstream().put(o1, now);
        });
